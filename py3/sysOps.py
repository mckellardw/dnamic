...
class TaskLog:
    ...
    def pop_overdue(self,maxruntimes):
        ...
        overdue = [((current_time > (maxruntimes[this_queue]*1.2 + start_time)) and (end_time<0)) for [start_time,end_time,this_queue] in zip(self.my_start_times,self.my_end_times,self.my_queue)]
    ...
    def write_log_to_file(self,task_log_file, alertfile = None):
        ...
        with open(globaldatapath + task_log_file,'w') as task_log_handle:
            for task,start,end,queue in zip(self.my_tasks,self.my_start_times,self.my_end_times,self.my_queue):
